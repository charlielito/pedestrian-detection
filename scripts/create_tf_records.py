...
with PythonPath("/code/models/research/"):
...
def dict_to_tf_example(
    data, label_map_dict, image_subdirectory, ignore_difficult_instances=False
):
  ...
  """
    ...
    for obj in data["object"]:
        ...
        coords = obj["bndbox"]
        x, y = float(coords["xmin"]), float(coords["ymin"])
        x2, y2 = float(coords["xmax"]), float(coords["ymax"])
        y2 = min(height, y2)
        x2 = min(width, x2)
        x = max(0, x)
        y = max(0, y)
...
def create_tf_record(
    output_filename, label_map_dict, annotations_dir, image_dir, examples
):
...
def main(_):
    ...
    examples_list = list(map(str, range(len(os.listdir(image_dir)))))
    ...
    val_output_path = "data/valid.record"
    create_tf_record(
        train_output_path, label_map_dict, annotations_dir, image_dir, train_examples
    )
    create_tf_record(
