...
if __name__ == "__main__":
    ...
    GT = pd.read_csv("data/TownCentre-groundtruth.top", header=None)
    indent = lambda x, y: "".join(["  " for _ in range(y)]) + x

    factor = 2
    train_size = len(os.listdir("images"))
    ...
    for frame_number in range(train_size):
        ...
        y2 = list(Frame[9])
        points = [
            [(round(x1_), round(y1_)), (round(x2_), round(y2_))]
            for x1_, y1_, x2_, y2_ in zip(x1, y1, x2, y2)
        ]

        with open(
            os.path.join(annotations_dir, str(frame_number) + ".xml"), "w"
        ) as file:
            file.write("<annotation>\n")
            file.write(
                indent("<filename>" + str(frame_number) + ".jpg" + "</filename>\n", 1)
            )
